;------------------------------------------------------------------------------------
; Flocking With Drawable Obstacles and Target model by Robert Margelli partially based on:
; Teahan, W. J. (2009).  Flocking With Obstacles NetLogo model. Artificial Intelligence. Ventus Publishing Aps. 
; and OBS NetLogo User Community model by Sifat Momen
; -----------------------------------------------------------------------------------

turtles-own [
  flockmates         ;; agentset of nearby turtles
  nearest-neighbor   ;; closest flockmate
  isMobile           ;; boolean for moving turtle
]
...
to create-world ;; observer procedure
  ...
  [
    ...
    set target-val -1
    create-field
    if (pxcor >= 30 and pxcor <= 35 and pycor >= 30 and pycor <= 35)
     [set target-val 1
       set pcolor red]
  ]  
...
to create-field ;;patches procedure
  set field (field + (Vt * exp (-1 * lambda * distancexy 31 31))) ;; field due to target
  if show-field? [set plabel round field  ]
end
...
to go
  if all? turtles [isMobile = false] [stop]
  go-to-target
  ask turtles [ flock ]
...
to make-obstacle
  if mouse-down?
  [ ask patches
    [ if ((abs (pxcor - mouse-xcor)) < psize) and ((abs (pycor - mouse-ycor)) < psize)
      [ set pcolor 65 ]]]
  display
end
...
; avoid anything nearby that is not black
    if (any? patches in-cone 4 vision-ang with [pcolor = 65])
    [ rt obs-steer ] ; head in opposite direction
end
...
410
446
Draw and erase collision patches
11
0.0
1
...
"" ""
PENS
"default" 1.0 0 -13840069 true "" "plot count turtles with [isMobile = true]"
...
Each bird is also attracted to the target by a potential field.
...
It is a further adaptation of a model by Thomas Christy and William John Teahan. Most of the information included here was provided by Uri Wilensky, with some modifications.
