; Robert Margelli (2014).  Flocking With Tree Avoidance and Target NetLogo model.
;as a further development of C.Bergey's adaptation of Uri Wilensky Flocking Model
...
to create-world ;; observer procedure
     ask patches[
       set field 0
       set dest-val -1
       create-field
       if (pxcor >= 31 and pxcor <= 35 and pycor >= -2 and pycor <= 2) [set dest-val 1]
       if dest-val = 1 [set pcolor red]
  ]
  setup-circle 0.5 population
end

to create-field ;;patches procedure
  set field (field + (Vt * exp (-1 * lambda * distancexy 32 0))) ;; field due to dest
 ; set plabel round field
  if (show-field? and (pxcor mod modulo = 0))  [set plabel round field  ]
end

to setup-circle [r n]
  set-default-shape turtles "default"
  create-ordered-boids n [
    setxy -30 0
    set color green - 2 + random 7
...
to go
  if all? boids [isMobile = false] [stop]
  go-to-tree
  ask boids [
    ;; This avoid-tree bit added by C. Bergey
    ifelse count trees in-cone tree-dist tree-sight-cone > 0 [
      avoid-tree
    ] [
      flock
    ]
...
to go-to-tree
  ...
  [ ifelse pen? [pd][pu]
    ...
    if isMobile[  ;finds out which patch ahead has the highest field value and takes the turtle to it
...
;
; Robert Margelli (2014).  Flocking With Drawable Obstacles and Target NetLogo model.
;
;
...
"" ""
PENS
"default" 1.0 0 -13840069 true "" "plot count boids with [isMobile = true]"
...
This model adds the following functionalities to C.Bergey's original model:
1. immediate flock formation;
2. potential field targeting.

## HOW IT WORKS

The birds' first priority is to avoid obstacles. Nevertheless the birds are brought to formation immediately after the "go" procedure.
The birds will follow a path to the target by influence of the potential field (visible
by toggling the "show-field?" switch).

## CREDITS AND REFERENCES

This model was created by Uri Wilensky and modified by Robert Margelli.
Obstacle avoidance functionality added by Christina Bergey, www.christinabergey.com,  
